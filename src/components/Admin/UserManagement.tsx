...
export const UserManagement: React.FC<UserManagementProps> = ({ onRefresh }) => {
  ...
  // Subscribe to real-time state changes (including Firestore updates)
  useEffect(() => {
  ...
  const handleManualCloudSync = async () => {
    ...
    try {
      const res = await firestoreSync.initialCloudSync();
      setUsersList(getUsers());
      setSyncFeedback(res.message || 'Sinkronisasi cloud berhasil.');
      onRefresh();
    } catch (e: any) {
      setSyncFeedback(`Gagal sinkronisasi: ${e.message}`);
    } finally {
      setIsSyncing(false);
  ...
  const handleConfirmDeleteUser = () => {
    ...
    firestoreSync.deleteUserFromCloud(userToDelete.id).catch(() => {});

    const admin = getCurrentUser();
    addAuditLog({
      actorId: admin?.id || 'admin',
      actorName: admin?.name || 'Admin',
      actorRole: admin?.role || 'ADMIN_PUSAT',
      actionType: 'USER_MANAGEMENT',
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 text-white shadow-xl space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
        <div>
          ...
          <p className="text-xs text-slate-400">
            Registrasi, edit data, hapus user, sinkronisasi cloud, dan pemantauan aktivitas login
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            disabled={isSyncing}
            className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-200 border border-slate-700 font-semibold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow transition cursor-pointer"
            title="Sinkronkan data akun secara real-time dengan Cloud Firestore"
          >
      ...
      {syncFeedback && (
        <div className="p-3 rounded-xl bg-sky-950/80 border border-sky-600/50 text-sky-200 text-xs flex items-center gap-2 animate-fadeIn">
          <CloudCheck className="w-4 h-4 text-sky-400 shrink-0" />
        ...
        <table className="w-full text-left text-xs text-slate-300">
          <thead className="bg-slate-950 text-slate-400 uppercase text-[10px] tracking-wider border-b border-slate-800 font-bold">
            <tr>
              ...
              <th className="p-3">Status & Aktivitas Login</th>
                ...
                <tr key={user.id} className="hover:bg-slate-800/50 transition">
                  ...
                  <td className="p-3">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`w-fit px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                          user.status === 'AKTIF'
                      ...
                      <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                        ...
                        <span>
                          {user.lastLogin
                            ? `Login: ${new Date(user.lastLogin).toLocaleString('id-ID', {
                                day: '2-digit',
                                month: 'short',
                                hour: '2-digit',
                                minute: '2-digit',
